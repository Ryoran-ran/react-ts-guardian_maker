export const guardianTitles = [
  '蒼天',
  '白煌',
  '紅蓮',
  '月影',
  '星巡',
  '雷鳴',
  '翠嵐',
  '霧灯',
  '天弓',
  '幽晶',
] as const

export const guardianDescriptors = [
  '暁光',
  '深翠',
  '燐火',
  '銀嵐',
  '霞月',
  '蒼雷',
  '煌星',
  '黒曜',
  '霜華',
  '陽炎',
  '天涙',
  '秘紋',
] as const

export const guardianConnectors = [
  'を纏う',
  'に護られし',
  'を統べる',
  'より現れし',
  'を宿す',
  'に選ばれし',
  'を渡る',
  'に祝福された',
] as const

export const guardianNameStarts = [
  'ア',
  'イ',
  'カ',
  'キ',
  'シ',
  'ソ',
  'ト',
  'ナ',
  'ヒ',
  'ミ',
  'ユ',
  'リ',
  'レ',
  'ロ',
  'セ',
  'ツ',
] as const

export const guardianNameMiddles = [
  'マ',
  'オ',
  'グ',
  'ラ',
  'ナ',
  'ス',
  'ヨ',
  'ツ',
  'ル',
  'ノ',
  'ハ',
  'ミ',
  'キ',
  'シ',
  'ト',
  'ヤ',
] as const

export const guardianNameEnds = [
  'ネ',
  'ト',
  'イ',
  'ラ',
  'ヤ',
  'ナ',
  'オ',
  'ア',
  'キ',
  'ル',
  'カ',
  'リ',
] as const

export const guardianNameSuffixes = [
  'フィア',
  'ノア',
  'リオン',
  'セナ',
  'アルト',
  'エル',
  'レイン',
  'ミア',
  'ヴィス',
  'ラ',
  'ティア',
  'ノエル',
  'セリア',
  'ヴェル',
  'リア',
  'シュ',
] as const

export const personalities = [
  '静かに状況を見抜く参謀肌',
  '困っている人を放っておけない献身家',
  '好奇心が強く未知を楽しむ探究者',
  '大胆に突き進む行動派',
  '仲間を鼓舞する陽気な統率者',
  '信念を曲げない孤高の守り手',
  '優雅に振る舞う芸術家気質',
  '粘り強く最後まで諦めない努力家',
] as const

export const personalityLeadings = [
  '冷静で',
  '朗らかで',
  '神秘的で',
  '大胆で',
  '繊細で',
  '誠実で',
  '穏やかで',
  '情熱的で',
  '理知的で',
  '柔軟で',
  '慎重で',
  '奔放で',
] as const

export const personalityMiddles = [
  '面倒見がよく',
  '責任感が強く',
  '空気を読むのがうまく',
  '自分の信念を大切にし',
  '周囲をよく観察し',
  '独自の感性を持ち',
  '仲間への気配りを忘れず',
  '感情の機微に敏く',
  '状況判断が早く',
  '物事を丁寧に積み上げ',
] as const

export const personalityEndings = [
  '仲間思い',
  '観察眼が鋭い',
  '芯が強い',
  '面倒見がよい',
  '少し気まぐれ',
  '直感に優れる',
  '決断力がある',
  '包容力がある',
  '努力を惜しまない',
  '意外と頑固',
  '発想が自由',
  '頼られることが多い',
] as const

export const favorites = [
  '朝焼けの空気',
  '澄んだ水辺',
  '古い書庫',
  '星明かりの散歩',
  '香木の香り',
  '鍛錬の時間',
  '祭礼の音色',
  '静かな雨音',
] as const

export const favoritePrefixes = [
  '静かな',
  '光る',
  '懐かしい',
  '澄んだ',
  'やわらかな',
  '神秘的な',
  '風の通る',
  '澄みきった',
  '古びた',
  '祝福された',
  'ぬくもりのある',
  '淡く輝く',
] as const

export const favoriteMiddles = [
  '森の',
  '水辺の',
  '夜空の',
  '書庫の',
  '祭壇の',
  '山道の',
  '朝露の',
  '月影の',
  '焚き火の',
  '花園の',
] as const

export const favoriteSubjects = [
  '森の香り',
  '月夜の散歩',
  '古代の書物',
  '祭礼の音色',
  '温かな湯気',
  '星降る空',
  '木漏れ日の静けさ',
  '澄んだ鐘の音',
  '焼きたての香ばしさ',
  '柔らかな布の感触',
  '小川のせせらぎ',
  '古い祈りの言葉',
] as const

export const weaknesses = [
  '騒がしすぎる場所',
  '不意打ちの褒め言葉',
  '湿気の多い日',
  '急な予定変更',
  '空腹のままの長話',
  '細かすぎる規則',
  '眠気を誘う午後',
  '曖昧な指示',
] as const

export const weaknessPrefixes = [
  '過度に',
  '妙に',
  'じわじわと',
  '不意に',
  '執拗に',
  '必要以上に',
  '思ったより',
  'じっと',
  'ひどく',
  'やたらと',
  '不自然に',
  'やけに',
] as const

export const weaknessMiddles = [
  '騒がしい',
  '湿った',
  '曖昧な',
  '張りつめた',
  '眠気を誘う',
  '空腹を招く',
  '急かされる',
  'ざわついた',
  '重たい',
  '乾ききった',
] as const

export const weaknessSubjects = [
  '騒がしい場所',
  '湿った空気',
  '曖昧な指示',
  '急な沈黙',
  '強い眠気',
  '空腹の時間',
  '長すぎる会話',
  '終わりの見えない待ち時間',
  '冷えた石畳',
  '雑然とした机',
  'まとまりのない報告',
  '急な予定変更',
] as const

export const weapons = [
  '月光の細剣',
  '雷紋の長槍',
  '星屑の法杖',
  '霊峰の戦斧',
  '守環の大盾',
  '白炎の弓',
  '風切りの双刃',
  '祈祷の宝珠',
] as const

export const abilityConditions = [
  '満月の夜に',
  '祈りを捧げると',
  '静寂に包まれると',
  '風が止んだ瞬間に',
  '朝露が光るころに',
  '強く願ったとき',
  '仲間と心を重ねると',
  '危機が迫ると',
  '星明かりの下で',
  '深く息を整えると',
] as const

export const abilityTargets = [
  '触れた影を',
  '視界に映る敵を',
  '周囲の空気を',
  '自らの武器を',
  '仲間の傷を',
  '足元の地面を',
  '飛来する力を',
  '閉ざされた道を',
  '流れる時間を',
  '揺らぐ心を',
] as const

export const abilityAdjectives = [
  '静かに',
  '鮮やかに',
  '一瞬で',
  '柔らかく',
  '力強く',
  '神秘的に',
  '容赦なく',
  '穏やかに',
  '密やかに',
  '確かに',
] as const

export const abilityEffects = [
  '凍らせる',
  '浄化する',
  '増幅する',
  '跳ね返す',
  '封じる',
  '導く',
  '癒やす',
  '眠らせる',
  '遅らせる',
  '変質させる',
] as const

export const quirkyPersonalities = [
  '頼まれると断れないが返事の前に少し遠くを見る',
  '慎重なくせに気になる貼り紙は必ず最後まで読む',
  '静かにしていたいのに整列した物を見ると揃えたくなる',
  'やさしいが布団の向きだけは妙に気にする',
  '落ち着いているが新しい文房具を見ると少しそわそわする',
  '面倒見がよいが急に褒められると視線が泳ぐ',
] as const

export const quirkyFavorites = [
  '少し冷めたお茶',
  '誰もいない時間の廊下',
  'うまく剥がせたシール',
  '雨上がりの乾ききらない空気',
  'ふわっと温かい洗濯物',
  'ぴったり閉まる引き出し',
] as const

export const quirkyWeaknesses = [
  '片方だけ見つからない靴下',
  'ぬるくなった炭酸',
  '閉まりそうで閉まらない引き出し',
  '急に話を振られる沈黙',
  'ほどけかけた袋の結び目',
  '行列の進み方が読めない時間',
] as const

export const quirkyAbilities = [
  '雨の日に洗濯物が早く乾く能力',
  '眠い朝に温かい飲み物が飲みごろになる能力',
  '急いでいるときに信号が少しだけ間に合いやすくなる能力',
  '誰にも見られていないときに探し物が見つかりやすくなる能力',
  '小さくくしゃみをすると靴ひもがほどけにくくなる能力',
  '帰り道で荷物の重さを少しだけ軽く感じる能力',
  '片づけを始めると引き出しがするっと閉まる能力',
  '寒い日に靴下が片方ずつちゃんと見つかる能力',
] as const

export const faceShapes = ['丸顔', '卵型', '菱形'] as const
export const skinColors = ['白磁', '蜜色', '褐色', '青白'] as const
export const hairstyles = ['短髪', '長髪', '逆立ち', '切り揃え'] as const
export const hairVolumes = ['少なめ', '標準', '多め'] as const
export const eyeSizes = ['小さめ', '標準', '大きめ'] as const
export const eyeShapes = ['丸い', '切れ長', 'つり目', '可愛い', 'クール'] as const
export const eyeSpacings = ['狭め', '標準', '広め'] as const
export const eyeColors = ['蒼', '翠', '琥珀', '紫紺'] as const
export const eyebrowShapes = ['まっすぐ', 'アーチ', 'きりり', 'たれ気味'] as const
export const eyebrowSizes = ['短め', '標準', '長め'] as const
export const eyebrowThicknesses = ['細い', '標準', '太い'] as const
export const eyebrowColors = ['黒', '茶', '銀', '紺'] as const
export const earShapes = ['丸耳', '葉耳', '尖り耳'] as const
export const earSizes = ['小さめ', '標準', '大きめ'] as const
export const noseShapes = ['丸い', 'すっきり', '短め'] as const
export const noseSizes = ['小さめ', '標準', '大きめ'] as const
export const neckLengths = ['短め', '標準', '長め'] as const
export const mouthShapes = ['微笑み', '凛々しい', '穏やか'] as const
export const mouthPositions = ['高め', '標準', '低め'] as const
export const mouthSizes = ['小さめ', '標準', '大きめ'] as const
export const shoulderWidths = ['狭め', '標準', '広め'] as const
export const bodyTypes = ['細身', '均整', '頑健'] as const
export const heights = ['低め', '標準', '高め'] as const
export const armThicknesses = ['細め', '標準', '太め'] as const
export const armAngles = ['内寄り', '標準', '外寄り'] as const
export const armLengths = ['短め', '標準', '長め'] as const
export const legLengths = ['短め', '標準', '長め'] as const
export const legThicknesses = ['細め', '標準', '太め'] as const
